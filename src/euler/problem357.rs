...
pub fn sample() {
    ...
    for i in 1..n {
        ...
        println!("Min Divisors of {}: {:?}", i, divisors_min(i, math::isqrt(i), &sieve));
        println!("Divisors of {} are prime: {}", i, divisors_are_prime(i, &sieve));
    }
...
pub fn sum_divisors_to(n: usize) -> usize {
    ...
    sieve.primes_from(2).map(|p| p - 1).take_while(|&x| x <= n).filter(|&d| divisors_are_prime(d, &sieve)).sum()
}
...
pub fn divisors_are_prime(n: usize, sieve: &primal::Sieve) -> bool {
    ...
    */
    ...
    divisors(n, sieve).iter().filter(|x| **x > 1 && **x <= root_n).all(|&d| sieve.is_prime(d + n/d))
}
